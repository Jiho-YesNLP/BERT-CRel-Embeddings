...
def read_mesh_descriptors():
    ...
    for rec in data.getiterator('DescriptorRecord'):
        ...
        pharmacological_actions = rec.findall(
            './/PharmacologicalAction/DescriptorReferredTo/DescriptorUI')
        see_also = rec.findall(
            './/SeeRelatedDescriptor/DescriptorReferredTo/DescriptorUI')
        # Add name, Pharmacological, see also relations
        mesh[ui] = {
            'name': name,
            'pa': [pa.text for pa in pharmacological_actions],
            'sa': [sa.text for sa in see_also]
        }
    return mesh
...
def generate_examples_mt(mesh_def, mt):
    logger.info('Generating relevance pairs from MeSH tree...')
    pos = []
    neg = []
    tree_numbers = []
    ...
    for i, tn in enumerate(tqdm(tree_numbers)):
        ...
        family = mt.find_meshes_within_distance(tn, distance=3)
        pos_ = [(1, me, node) for node in family if me != node]
        neg_ = [(0, me, node) for node in
                random.choices(list(mesh_def.keys()), k=len(pos_))]
        pos.extend(pos_)
        neg.extend(neg_)

    logger.info('Adding PharmacologicalAction and SeeAlso relations...')
    for ui, me in mesh_def.items():
        if len(me['pa']) > 0 or len(me['sa']) > 0:
            pa_ = [(1, ui, msh) for msh in me['pa']]
            sa_ = [(1, ui, msh) for msh in me['sa']]
            pos_ = pa_ + sa_
            neg_ = [(0, ui, msh) for msh in
                    random.choices(list(mesh_def.keys()), k=len(pos_))]
            pos.extend(pos_)
            neg.extend(neg_)
