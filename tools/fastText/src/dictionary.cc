...
void Dictionary::add(const std::string& w) {
  ...
  if (word2int_[h] == -1) {
    entry e;
    e.word = w;
    // We want to include all MeSH codes that starts with 'd'
    // Reset the counter to bypass the minCount condition
    if (w.length() >= 4 && w[0] == 'd' &&
        0 == w.compare(w.length()-4, 4, "mesh")){
...
void Dictionary::initNgrams() {
  for (size_t i = 0; i < size_; i++) {
    ...
    std::string w = words_[i].word;
    bool isEntity = (w.length() >= 4 && 0 == w.compare(w.length()-4, 4, "mesh"));
    if (!isEntity && w != EOS) {
