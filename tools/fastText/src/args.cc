...
Args::Args() {
  ...
  minCountLabel = 0;      // minimal number of label occurrences
  neg = 5;                // number of negative sampled
  wordNgrams = 1;         // max length of word-level ngrams
  loss = loss_name::ns;   // loss function
  model = model_name::sg; // model name
  bucket = 2000000;       // number of buckets for hashing char ngram
  minn = 3;               // minimal length of char ngram
  maxn = 6;               // maximal length of char ngram
  thread = 12;            // number of threads
  lrUpdateRate = 100;     // change the learning rate per N token counts
  t = 1e-4;               // sampling threshold
  label = "__label__";    // label prefix
  verbose = 2;            // verbose level
  pretrainedVectors = ""; // pretrained word vectors for supervised learning
  saveOutput = false;     // whether output params should be saved
